$(function() {
	...
	if (localStorage.autozoom === undefined) {
		...
		localStorage.pxbHelp = "true";//培训班招生
		$("#install").show();
		init();
		return;
	}
	...
	function showTitle() {
		this.innerText = $(this).parent()[0].id.replace(/\b(\w)/, function(m) {
			return m.toUpperCase();
		});
	}

	$("#vocabulary>h1").click(function() {
		var count = 1;
		return function(event) {
			(count++ % 2 ? showCount : showTitle).call(this)
		}
	}());

	$("#history>h1").click(function() {
		var count = 1;
		return function(event) {
			(count++ % 2 ? showCount : showTitle).call(this)
		}
	...
	$('#search').keyup(function() {
		...
		// 语音输入，x-webkit-speech已经不推荐使用了，所以去掉
		// updateList(this.value);
	}).on('paste', function() {
		var self = this;
		setTimeout(function() {
			updateList(self.value);
		});
	});
	...
	// 单击任何列表里的词时切换单词
	$("div.word-list").on('click', ">div", function() {
	...
	// 删除生词表里的单词
	$("#vocabulary-content").on('click', ">div>span.del", function() {
		removeVocabulary($(this).parent().text());
		return false;
	});

	// 删除历史记录里的单词
	$("#history-content").on('click', ">div>span.del", function() {
		removeHistory($(this).parent().text());
		return false;
	});

	// 双击历史记录的单词把其加入生词表
	$("#history-content").on('dblclick', ">div", function() {
		addVocabulary($(this).attr('word'));
	}).on("dblclick", "span.del", function(event) {
	...
	$font_style.append("@import url(http://fonts.googleapis.com/css?family=%s);\n".format(eng_fonts.map(function(o) {
		return o.replace(/ /ig, "+")
	}).join("|")));
	...
	if (localStorage.onlyee == "true") {
		...
		document.body.classList.add('onlyee');
	}
	//仅英英解释下长按1s显示中文意思
	$content.on("mousedown", "span.oa_d,span.oa_ud,span.oa_x", function() {
		if (localStorage.onlyee != "true") return; //仅英英解释下生效
		var self = this;
		this.timeout = setTimeout(function() {
			$("[class*=chn]", self).toggle();
		}, 1000);
	}).on("mouseup", "span.oa_d,span.oa_ud,span.oa_x", function() {
		//取消上面的定时器
		clearTimeout(this.timeout);
	}).on("mousedown", "span.oa_d,span.oa_ud,span.oa_x", function() {
		//英英解释及例句发音
		...
		// soundsrc = 'https://translate.google.com.hk/translate_tts?ie=UTF-8&q=%s&tl=en&client=t&prev=input'.format(encodeURI(soundtext.trim()))
	...
	*/
	...
	//培训相关
	$('#fe-training').click(showPxbAd)
	if (localStorage.pxbHelp === 'true') {
		showPxbAd()
	}
	//培训相关结束
...
window.document.__defineGetter__('referrer', function() {
	return "https://translate.google.com.hk/";
});
...
function showPxbAd(){
	$.get('https://xieranmaya.github.io/fe-training/oald.md').then(function(mdText) {
		$('#pxb article')[0].innerHTML = marked(mdText)

		$('#pxb').show().css('opacity',0).animate({opacity:1},500)
		$('#pxb span.close').click(function(){
			localStorage.pxbHelp = 'false'
		})
	})
}
...
function setEngFont(font) {
	...
	$("#font-style").append('html,body,button,input{font-family:"' + font + '";}')
...
function showWord(word, callback) {
	...
	var pronbase = "https://cddm.me/google-dict-sounds/%s.mp3";
	var pronbase1 = "https://cddm.me/google-dict-sounds/%s@1.mp3";
...
window.onhashchange = function() {
	var word = location.hash.substr(1);
	showWord(decodeURIComponent(word));
};
