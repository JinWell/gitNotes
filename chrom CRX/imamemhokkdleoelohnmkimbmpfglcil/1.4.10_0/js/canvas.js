...
var Canvas = (function() {
  ...
  var calculateArrowCoordinates = function(arrowWidth, arrowHeight, startX, startY, endX, endY) {
    var p1 = function() {
      var x = startX - endX;
      var y = startY - endY;
  ...
  return {
    drawStrokeRect: function(ctx, color, x, y, width, height, lineWidth) {
    ...
    drawEllipse: function(ctx, color, x, y, xAxis, yAxis, lineWidth, type) {
      var startX = x + xAxis;
      var startY = y;
      ctx.beginPath();
      ctx.lineWidth = lineWidth;
    ...
    setText: function(ctx, text, color, fontSize, fontFamily, lineHeight, x, y, width) {
    ...
    drawLine: function(ctx, color, lineCap, lineWidth, startX, startY, endX, endY) {
      ctx.beginPath();
      ctx.moveTo(startX, startY);
      ctx.strokeStyle = color;
      ctx.lineWidth = lineWidth;
    ...
    drawArrow: function(ctx, color, lineWidth, arrowWidth, arrowHeight, lineCap, startX, startY, endX, endY) {
    ...
    drawRoundedRect: function(ctx, color, x, y, width, height, radius, type) {
      ctx.beginPath();
    ...
    blurImage: function(realCanvas, simulateCanvas, layerId, startX, startY, endX, endY) {
      ...
      // 之前的 + 10 加个锤子啊
      simulateCanvas.width = document.getElementById(layerId).clientWidth;
      simulateCanvas.height = document.getElementById(layerId).clientHeight;
      ...
      try {
        ...
        if (window.devicePixelRatio > 1) {
          ctx.drawImage(realCanvas, x, y, width * 2, height * 2, 0, 0, width, height);
        } else {
          ctx.drawImage(realCanvas, x, y, width2, height2, 0, 0, width, height);
        }
      } catch(error) {
