  ...
  */
  onMouseMove: function(event) {
    ...
    if(photoshop.dragFlag) {
      ...
      var startX = photoshop.startX;
      var startY = photoshop.startY;
      var endX = photoshop.endX;
      var endY = photoshop.endY;
      if (window.devicePixelRatio > 1) {
        startX = startX * window.devicePixelRatio;
        startY = startY * window.devicePixelRatio;
        endX = endX * window.devicePixelRatio;
        endY = endY * window.devicePixelRatio;
      }
      ...
      if (photoshop.flag == 'line' || photoshop.flag == 'arrow') {
        photoshop.drawLineOnMaskCanvas(startX,
                                       startY,
                                       endX,
                                       endY,
                                       'lineDrawing',
        ...
        photoshop.drawEllipseOnMaskCanvas({
          startX,
          startY,
          endX,
          endY,
          type: 'drawing',
          layerId: photoshop.layerId
        });
      }
    }
    event.stopPropagation();
  ...
  */
  draw: function() {
    ...
    for (var j = 0; j < photoshop.markedArea.length; j++) {
      ...
      var startX = mark.startX;
      var startY = mark.startY;
      var endX = mark.endX;
      var endY = mark.endY;
      var width  = mark.width;
      var height = mark.height;
      var color  = mark.color;
      var fontSize = mark.fontSize;
      if (window.devicePixelRatio > 1) {
        ...
        endX = endX * window.devicePixelRatio;
        endY = endY * window.devicePixelRatio;
        // 先取整，后做乘法，可能将 0.3 ＊ 2 这种忽略掉，其它几个可以不管，但 width, height 会导致如 fillText 意外换行，故在此 + 1
      ...
      switch(mark.flag) {
        ...
        case 'ellipse':
          ...
          if (window.devicePixelRatio > 1) {
            x = x * window.devicePixelRatio;
            y = y * window.devicePixelRatio;
            xAxis = xAxis * window.devicePixelRatio;
            yAxis = yAxis * window.devicePixelRatio;
          }
        ...
        case 'text':
          for (var i = 0; i < mark.context.length; i++) {
            Canvas.setText(context, mark.context[i], color,
                           fontSize + 'px', 'arial', fontSize,
                           x, y + fontSize * i, width);
          }
          break;
        case 'blur':
  ...
  drawEllipseOnMaskCanvas: function(config) {
    var startX = config.startX;
    var startY = config.startY;
    var endX = config.endX;
    var endY = config.endY;
    ...
    var x = (startX + endX) / 2;
    var y = (startY + endY) / 2;
    var xAxis = Math.abs(endX - startX) / 2;
    var yAxis = Math.abs(endY - startY) / 2;
    Canvas.drawEllipse(ctx, photoshop.color, x, y, xAxis, yAxis, 3, photoshop.highlightType);
    if (type == 'end') {
      var offsetLeft = parseInt($(layerId).style.left);
      var offsetTop = parseInt($(layerId).style.top);
      var startX = startX - offsetLeft;
      var startY = startY - offsetTop;
      var newEndX = endX - offsetLeft;
      var newEndY = endY - offsetTop;
      x = (startX + newEndX) / 2;
      y = (startY + newEndY) / 2;
      xAxis = Math.abs(newEndX - startX) / 2;
      yAxis = Math.abs(newEndY - startY) / 2;
      var cavCopy = photoshop.createCanvas(layerId);
      cavCopy.width = Math.abs(endX - startX);
      cavCopy.height = Math.abs(endY - startY);
      var ctxCopy = cavCopy.getContext('2d');
      Canvas.drawEllipse(ctxCopy, photoshop.color, x, y, xAxis, yAxis, 3, photoshop.highlightType);
      ctx.clearRect(0, 0, $('mask_canvas').width, $('mask_canvas').height);
    }
  },
  ...
  initCanvas: function() {
    ...
    if (window.devicePixelRatio > 1) {
      ...
      photoshop.canvas.style.width = photoshop.canvas.width / window.devicePixelRatio + 'px';
      photoshop.canvas.style.height = photoshop.canvas.height / window.devicePixelRatio + 'px';
      $('mask_canvas').style.width = $('canvas').style.width = $('canvas').width / window.devicePixelRatio + 'px';
      $('mask_canvas').style.height = $('canvas').style.height = $('canvas').height / window.devicePixelRatio + 'px';
    }
