...
window.addEventListener('load', function () {
    ...
    function initRating() {
        ...
        function setRating(n) {
            for (i = 0; i < 5; ++i) {
                if (i <= n)
                    stars[i].classList.add('starHighlighted');
                else
                    stars[i].classList.remove('starHighlighted');
            }
        }

        var parentElem = document.getElementById("divRating");

        for (var i = 0; i < 5; ++i) {
            var star = document.createElement('li');
            star.classList.add('glyphicon');
            star.classList.add('glyphicon-star');
            star.classList.add('starRating');
            star.setAttribute('data-index', i.toString());
            stars.push(star);
            star.addEventListener("click", function(e){
                var nRating = parseInt(e.target.getAttribute('data-index')) + 1;

                if (nRating > 4 && getOption(cRatingFilled, "") !== "true") {
                    backgroundPage.openURL("http://getfireshot.com/like.php?browser=" + (isOpera() ? "op" : "ch") + "&ver=" + backgroundPage.extVersion);
                    $('#modalSetRating').modal('hide');
                }
                else {
                    localStorage[cRatingFilled] = "true";
                    $('#divRating').hide();
                    $('#divRatingThankYou').fadeIn();
                    setTimeout(function() {$('#modalSetRating').modal('hide');}, 2000);
                }
            });

            star.addEventListener('mouseover', function(e) {
                setRating(parseInt(e.target.getAttribute('data-index')));
            });
            ...
            //parentElem.appendChild(star);
            parentElem.append(star);
        }

        $('#modalSetRating').on('show.bs.modal', function (e) {
            $('#divRatingThankYou').hide();
            $('#divRating').show();
        })
